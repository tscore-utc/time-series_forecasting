my.masePlotter.fun <- function(df, forecastList) {
  ...
  for (i in 1:length(myList)) {
    ...
    df <- pivot_longer(df, cols = -Dates, names_to = "Model", values_to = "MASE")
    
    # Convert MAPE/MASE into a numeric
    df$MASE <- as.numeric(df$MASE) # change according to if you're plotting MAPE/MASE
    
    # Plot the performance measures over time (date is first observation of testing data)
    performancePlot <- ggplot(df, aes(x = as.Date(Dates), y = MASE, color = Model)) +
      geom_line() +
      labs(x = "Testing Window Start Date", y = "MASE", title = "Change in MASE with Change in Testing Window Start Date") +
    ...
    output_filename <- file.path("/Users/ashleymorgan/Documents/previous research/forecasting project/major revision/plots/slidingPostMASE", paste0("sliding_MASE_", names(forecastList[i]), ".png"))
